...
export function diff() {
  ...
  return function (options = {}) {
    ...
    for (let i = periods; i < values.length; i++) {
      ...
      // Check for strings that can be converted to numbers
      const numCurrent =
        typeof currentValue === 'string' ? Number(currentValue) : currentValue;
      const numPrevious =
      ...
      if (
        ...
        typeof numCurrent !== 'number' ||
        typeof numPrevious !== 'number' ||
        Number.isNaN(numCurrent) ||
        Number.isNaN(numPrevious)
      ) {
        result[i] = NaN;
      } else {
