 ...
 *
 * @param {DataFrame} df - DataFrame to apply rolling window to
 ...
 */
export function rolling(df, options) {
  const {
    window,
    aggregations = {},
    center = false,
  ...
  // Keep columns that are not being aggregated
  for (const colName of df.columns) {
    if (!aggregations[colName]) {
