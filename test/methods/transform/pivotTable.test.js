...
import {
  ...
  max,
  min,
} from '../../../src/methods/transform/pivot.js';
...
describe('DataFrame.pivotTable', () => {
  ...
  test('creates a pivot table with multiple aggregation functions as an array', () => {
    ...
    expect(Array.from(result.frame.columns['region_North.sales_sum'])).toEqual([
      30, 15,
    ]); // 10+20, 15
    expect(Array.from(result.frame.columns['region_South.sales_sum'])).toEqual([
      30, 60,
    ]); // 30, 25+35

    // Check the values for mean aggregation
    expect(Array.from(result.frame.columns['region_North.sales_mean'])).toEqual(
      [15, 15],
    ); // (10+20)/2, 15/1
    expect(Array.from(result.frame.columns['region_South.sales_mean'])).toEqual(
      [30, 30],
    ); // 30/1, (25+35)/2
  ...
  test('creates a pivot table with multiple aggregation functions as an object', () => {
    ...
    expect(Array.from(result.frame.columns['region_North.average'])).toEqual([
      10, 15,
    ]); // mean
    expect(Array.from(result.frame.columns['region_North.minimum'])).toEqual([
      10, 15,
    ]); // min
    expect(Array.from(result.frame.columns['region_North.maximum'])).toEqual([
      10, 15,
    ]); // max
    ...
    expect(Array.from(result.frame.columns['region_South.average'])).toEqual([
      20, 25,
    ]); // mean
    expect(Array.from(result.frame.columns['region_South.minimum'])).toEqual([
      20, 25,
    ]); // min
    expect(Array.from(result.frame.columns['region_South.maximum'])).toEqual([
      20, 25,
    ]); // max
