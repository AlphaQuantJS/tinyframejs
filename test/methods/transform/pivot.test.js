...
describe('DataFrame.pivot', () => {
  test('creates a pivot table with default aggregation function (sum)', () => {
    ...
    expect(Array.from(result.frame.columns['region_North'])).toEqual([10, 15]);
    expect(Array.from(result.frame.columns['region_South'])).toEqual([20, 25]);
    expect(Array.from(result.frame.columns['region_East'])).toEqual([30, 35]);
    expect(Array.from(result.frame.columns['region_West'])).toEqual([40, 45]);
  });
  ...
  test('uses built-in mean aggregation function', () => {
    ...
    expect(Array.from(result.frame.columns['region_North'])).toEqual([15, 15]); // (10+20)/2, 15/1
    expect(Array.from(result.frame.columns['region_South'])).toEqual([30, 30]); // 30/1, (25+35)/2
  });
  ...
  test('uses built-in count aggregation function', () => {
    ...
    expect(Array.from(result.frame.columns['region_North'])).toEqual([2, 1]); // 2 entries for Product A, 1 for Product B
    expect(Array.from(result.frame.columns['region_South'])).toEqual([1, 2]); // 1 entry for Product A, 2 for Product B
  });
  ...
  test('handles null values correctly', () => {
    ...
    const df = DataFrame.create({
      product: ['Product A', 'Product A', 'Product B', null],
      region: ['North', 'South', 'North', 'South'],
      sales: [10, 20, 15, 25],
    });
