...
import {
  matches,
  register,
} from '../../../../src/methods/series/filtering/matches.js';
...
describe('Series.matches', () => {
  ...
  test('throws error when pattern is not provided', () => {
    const series = new Series(['apple', 'banana', 'cherry']);
    expect(() => series.matches()).toThrow(
      'Regular expression pattern must be provided',
    );
    expect(() => series.matches(null)).toThrow(
      'Regular expression pattern must be provided',
    );
  });
